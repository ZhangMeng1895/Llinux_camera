...
static void MyErrorExit(j_common_ptr ptCInfo)
{
	...
	struct MyErrorMgr *ptMyErr = (struct MyErrorMgr *)ptCInfo->err;
 ...
 * -----------------------------------------------
 ***********************************************************************/
// static int GetPixelDatasFrmJPG(PT_FileMap ptFileMap, struct PixelDatas * ptPixelDatas)
/* 把内存里的JPEG图像转换为RGB图像 */
static int Mjpeg2RgbConvert(struct videobuf *ptVideoBufIn, struct videobuf *ptVideoBufOut)
{
	...
	struct MyErrorMgr tJerr;
	struct PixelDatas *ptPixelDatas = &ptVideoBufOut->pdata;
	...
	/* 把数据设为内存中的数据 */
	jpeg_mem_src_tj(&tDInfo, ptVideoBufIn->pdata.aucPixelDatas, ptVideoBufIn->pdata.iTotalBytes);
...
static int Mjpeg2RgbConvertExit(struct videobuf *ptVideoBufOut)
{
	if (ptVideoBufOut->pdata.aucPixelDatas)
	{
		free(ptVideoBufOut->pdata.aucPixelDatas);
		ptVideoBufOut->pdata.aucPixelDatas = NULL;
	}
	return 0;
}

/* 构造 */
static struct convert_operations mjpg_c_ops = {
	.name = "mjpeg2rgb",
	.is_support = isSupportMjpeg2Rgb,
	.convert = Mjpeg2RgbConvert,
	.convert_exit = Mjpeg2RgbConvertExit,
};

/* 注册 */
int Mjpeg2RgbInit(void)
{
	return register_convert_ops(&mjpg_c_ops);
}
